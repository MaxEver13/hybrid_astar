...
void AStarAlgorithm::setFootprint(const std::vector<geometry_msgs::Point>& footprint, bool use_radius)
{
...
AStarAlgorithm::NodePtr AStarAlgorithm::addToGraph(
  ...
  const float & tolerance)
{
  ...
  ROS_WARN("iterations: %d, _queue size: %d", iterations, _queue.size());
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (isGoal(current_node)) {
      ...
      {
        ROS_WARN("approach_iterations: %d", approach_iterations);
  ...
  ROS_WARN("create path return false iterations: %d _queue size: %d...", iterations, _queue.size());
  return false;
}
...
AStarAlgorithm::NodePtr AStarAlgorithm::getAnalyticPath(
  const NodePtr & node,
...
bool AStarAlgorithm::backtracePath(NodePtr & node, CoordinateVector & path)
{
  if (!node->parent) {
    ROS_WARN("backtracePath return false");
    return false;
  }
...
AStarAlgorithm::NodePtr & AStarAlgorithm::getStart()
{
  return _start;
}

AStarAlgorithm::NodePtr & AStarAlgorithm::getGoal()
{
  return _goal;
}
...
float & AStarAlgorithm::getAccumulatedCost(NodePtr & node)
{
  return node->getAccumulatedCost();
}

float AStarAlgorithm::getHeuristicCost(const NodePtr & node)
{
...
unsigned int & AStarAlgorithm::getSizeDim3()
{
  return _dim3_size;
}


typename AStarAlgorithm::NodePtr AStarAlgorithm::tryAnalyticExpansion(
